	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPut:
			...
			if err := service.UpdateConfig(config); err != nil {
				log.Printf("update telemetry config: %v", err)
				writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "telemetry configuration could not be saved"})
				return
			}
			writeJSON(w, http.StatusOK, service.Config())
		default:
...
<head>
  ...
  <title>DronService — Телеметрия</title>
  <style>
...
<main class="app-main">
  <div class="page-heading"><h1>Телеметрия · MAVLink</h1><span id="link-badge" class="metric-card" style="padding:8px 12px;margin:0">Линк: …</span></div>
  <div id="telemetry"><p class="empty">Получение телеметрии…</p></div>
  <section class="config-card">
    <h2>Настройки на этом дроне</h2>
    <p class="meta" style="margin-top:6px">Сохраняются в <code>/var/lib/dronservice/mavlink.json</code>. Источник MAVLink должен слать телеметрию по UDP на указанный адрес.</p>
    <form id="fc-config-form">
      <div class="config-grid">
        <label class="toggle-row"><input id="fc-enabled" type="checkbox"> Включить MAVLink</label>
        <label>UDP адрес<input id="fc-udp-addr" type="text" value="0.0.0.0:14550" required></label>
        <label>Target system ID<input id="fc-target-system" type="number" min="0" max="255" value="0"></label>
        <label>Out system ID<input id="fc-out-system" type="number" min="1" max="255" value="255"></label>
        <label>Таймаут линка<input id="fc-link-timeout" type="text" value="5s"></label>
        <label>Интервал сообщений<input id="fc-message-interval" type="text" value="500ms"></label>
      </div>
      <div class="actions"><button type="submit">Сохранить</button></div>
      <div id="fc-save-notice" class="notice" hidden></div>
    </form>
  </section>
  <script>
    ...
    const telemetry=document.querySelector('#telemetry'),linkBadge=document.querySelector('#link-badge'),form=document.querySelector('#fc-config-form');
    ...
    function fillForm(cfg){
      document.querySelector('#fc-enabled').checked=!!cfg.enabled;
      document.querySelector('#fc-udp-addr').value=cfg.udpAddr||'0.0.0.0:14550';
      document.querySelector('#fc-target-system').value=cfg.targetSystemId??0;
      document.querySelector('#fc-out-system').value=cfg.outSystemId||255;
      document.querySelector('#fc-link-timeout').value=cfg.linkTimeout||'5s';
      document.querySelector('#fc-message-interval').value=cfg.messageInterval||'500ms';
      formDirty=false;
      formLoaded=true;
    }
    ...
    async function load(){
      try{
        const response=await fetch('/api/telemetry/status',{cache:'no-store'});
        const data=await response.json();
        if(!response.ok)throw new Error(data.error||'MAVLink недоступен');
        renderTelemetry(data);
        if(!formLoaded||!formDirty)fillForm(data.config||{});
      }catch(error){
        telemetry.innerHTML='<p class="error-box">'+escapeHTML(error.message)+'</p>';
      }
    }
    async function saveConfig(event){
      ...
      try{
        const response=await fetch('/api/telemetry/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
        const data=await response.json();
