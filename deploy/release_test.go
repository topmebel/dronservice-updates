...
func TestDeploymentManifestIsVersionedAndComplete(t *testing.T) {
	...
	for _, required := range []string{"dronservice-linux-arm64", "dronservice-camera-network-helper", "dronservice-camera-network.service", "install-mediamtx.sh", "mediamtx.service", "dronservice.service", "dronservice-release.pub", "migrate-local-settings.sh"} {
