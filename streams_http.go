...
document.querySelector('#streams-body').onclick=async event=>{const previewButton=event.target.closest('[data-stream-preview]');if(previewButton){event.stopPropagation();openStreamPreview(previewButton);return}const button=event.target.closest('.copy-path');if(!button)return;event.stopPropagation();try{if(navigator.clipboard){await navigator.clipboard.writeText(button.dataset.copy)}else{const area=document.createElement('textarea');area.value=button.dataset.copy;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();if(!document.execCommand('copy'))throw new Error('copy failed');area.remove()}const old=button.textContent;button.textContent='✓';setTimeout(()=>button.textContent=old,1200)}catch(error){alert('Не удалось скопировать RTSP path')}};document.querySelector('#streams-body').ondblclick=event=>{if(event.target.closest('button'))return;const row=event.target.closest('tr[data-name]');if(row)openEditor(row)};async function refreshStreams(){if(dialog.open||previewDialog.open)return;try{const html=await fetch('/streams',{cache:'no-store'}).then(r=>r.text());const next=new DOMParser().parseFromString(html,'text/html').querySelector('#streams-body');if(next)document.querySelector('#streams-body').replaceChildren(...next.children)}catch(error){}}setInterval(refreshStreams,5000);nameInput.oninput=refreshDetails;sourceSelect.onchange=refreshDetails;document.querySelector('#stream-name-info').onclick=()=>streamNameInfoDialog.showModal();document.querySelector('#stream-name-info-close').onclick=()=>streamNameInfoDialog.close();document.querySelector('#add').onclick=()=>openEditor();document.querySelector('#close').onclick=()=>dialog.close();document.querySelector('#save').onclick=async()=>{if(!nameInput.checkValidity()){nameInput.reportValidity();return}if(!sourceSelect.value){alert('Выберите источник');return}const response=await fetch('/api/stream-configs',{method:editing?'PATCH':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:nameInput.value,originalName:originalName,sourceId:sourceSelect.value})});if(!response.ok){alert((await response.json()).error);return}location.reload()};remove.onclick=async()=>{const name=originalName||nameInput.value;if(!confirm('Удалить стрим '+name+'?'))return;const response=await fetch('/api/stream-configs?name='+encodeURIComponent(name),{method:'DELETE'});if(!response.ok){alert('Не удалось удалить стрим');return}location.reload()};
